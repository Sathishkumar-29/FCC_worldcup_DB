...
if [[ $OPPONENT != "opponent" ]]
  ...
  then 
    if [[ $YEAR != "year" ]]
    then
      INSERT_GAMES="$($PSQL "insert into games(year, round,winner_id,opponent_id,winner_goals,opponent_goals) values ($YEAR,'$ROUND','$TEAM_ID_W','$TEAM_ID_O',$WINNER_GOAL,$OPPONENT_GOAL)")"
